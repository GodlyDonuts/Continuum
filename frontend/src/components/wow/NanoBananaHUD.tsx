...
export const NanoBananaHUD = ({ targetId, deltaX = 0, deltaY = 0, rotation = 0 }: NanoBananaHUDProps) => {
    return (
        <div className="absolute inset-0 pointer-events-none z-30">
            ...
            <div className="absolute top-1/3 left-1/2 w-48 h-32 -translate-x-1/2">
                ...
                <div
                    className="absolute top-0 right-0 w-32 h-[1px] bg-white/40 origin-left"
                    style={{ transform: `translateX(12px) translateY(12px) rotate(${rotation - 15}deg)` }}
                ></div>

                <div className="absolute top-[-30px] right-[-140px] text-white/80 font-mono text-[9px] bg-black/50 px-1">
                    OBJ: {targetId ?? 'SCANNING...'}
                </div>

                {/* Technical Label */}
                <div className="absolute -bottom-6 left-0 flex gap-4 font-mono text-[9px] text-white/60">
                    <span>DY: {deltaY > 0 ? '+' : ''}{deltaY.toFixed(3)}</span>
                    <span>DX: {deltaX > 0 ? '+' : ''}{deltaX.toFixed(3)}</span>
                </div>

                {/* Status */}
                <div className="absolute -top-6 left-0 flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 ${targetId ? 'bg-orange animate-pulse' : 'bg-white/20'}`}></div>
                    <span className="font-mono text-[9px] text-white/80 tracking-widest">{targetId ? 'TRACKING_LOCK' : 'SEARCHING'}</span>
                </div>
            </div>
        </div>
    );
